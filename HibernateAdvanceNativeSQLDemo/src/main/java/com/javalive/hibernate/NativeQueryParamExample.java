	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Object[]> employees = session.createNativeQuery("SELECT * FROM employee where dept_id=?")
					...
					.addScalar("designation", StringType.INSTANCE).setParameter(1, 11L).list();
			...
			List<Department> departments = session
					.createNativeQuery("SELECT * FROM department where name like :deptName").addEntity(Department.class)
					.setParameter("deptName", "S%").list();
